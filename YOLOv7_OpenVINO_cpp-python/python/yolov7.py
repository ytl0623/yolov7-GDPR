...
class YOLOV7_OPENVINO(object):
    def __init__(self, model_path, device, pre_api, batchsize):
        ...
        self.out = cv2.VideoWriter("3_IR_object.mp4",
    ...
    def draw(self, img, boxinfo):
        ...
        self.out.write(img)
        #cv2.imwrite( f"{self.count}.jpg", img)
    ...
    def postprocess(self, infer_request, info):
        ...
        for batch_id in range(self.batchsize):
            ...
            output.append(self.sigmoid(
                infer_request.get_output_tensor(0).data[batch_id].reshape(-1, self.size[0] * 3, 5 + self.class_num)))
            output.append(self.sigmoid(
                infer_request.get_output_tensor(1).data[batch_id].reshape(-1, self.size[1] * 3, 5 + self.class_num)))
            output.append(self.sigmoid(
                infer_request.get_output_tensor(2).data[batch_id].reshape(-1, self.size[2] * 3, 5 + self.class_num)))
    ...
    def infer_cam(self, source):
        ...
        cap = cv2.VideoCapture("C:\\Users\\User\\Downloads\\yolov7\\2.mp4")
        ...
        fps = self.count / (end_time - start_time)
